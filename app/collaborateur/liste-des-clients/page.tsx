...
const Page = () => {
  ...
  const fetchUserData = async () => {
    try {
      const response = await fetch(`/api/admin/user?userType&creator=${id}`);
  ...
  return (
    <div className="min-h-screen flex-col justify-between lg:pt-[70px] pt-[60px]">
      <BannerTitle tile={"Liste des clients"} paragraph={""} />
      <Breadcrumb
        paths={[
          { name: "Accueil", url: "/collaborateur/mon-compte" },
          { name: "Mon compte", url: "/collaborateur/mon-compte" },
          { name: "Liste des clients", url: "#" },
        ]}
      />
      <div className="lg:pt-20 p-10 max-w-[1280px] mx-auto">
        <Title>Liste des clients</Title>
        <div className="flex lg:justify-end justify-center lg:mt-0 mt-4">
          <AddButton
            title="AJOUTER UN CLIENT"
            destination="/collaborateur/nouveau-client"
          />
        </div>
            ...
            <div className="mt-10 flex justify-start gap-4 flex-wrap">
              {usersToShow.map((user) => (
                <UserCard
                  ...
                  phone={user.phone}
                  email={user.email}
                  emailSecondary={user.email2}
                  phoneSecondary={user.phoneSecondaire}
