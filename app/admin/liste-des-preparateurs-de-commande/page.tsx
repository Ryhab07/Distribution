...
const Page = () => {
  ...
  const fetchUserData = async () => {
    try {
      const response = await fetch("/api/admin/user?userType=picker");
      if (!response.ok) {
  ...
  return (
    <div className="min-h-screen flex-col justify-between lg:pt-[70px] pt-[60px]">
      <BannerTitle tile={"Liste des préparateurs de commande"} paragraph={""} />
      <Breadcrumb
        paths={[
          { name: "Accueil", url: "/admin/mon-compte" },
          { name: "Mon compte", url: "/admin/mon-compte" },
          { name: "Liste des préparateurs de commande", url: "#" },
        ]}
      />
      <div className="lg:pt-20 p-10 max-w-[1280px] mx-auto">
        <div className="flex justify-end">
          <AddButton
            title="AJOUTER UN PREPARATEUR DE COMMANDE"
            destination="/admin/nouveau-preparateurs-de-commande"
          />
        </div>
