...
import {
  USERPRO_MOBILE_NAVBAR_ELEMENTS,
  USERPRO_NAVBAR_ELEMENTS,
} from "@/constants";
import { AlignJustify, ShoppingCart } from "lucide-react";
import Logo from "./logo";
import AuthButtonMobile from "./authButtonMobile";
import logos from "@/assets/logo/Logo-Larna.png";
...
const NavBarUserPro = () => {
  ...
  return (
    <header>
      <div>
        <nav className="relative lg:h-[110px] h-[60px] flex lg:flex-row justify-end items-center w-full  bg-devinovBleu z-10 py-5">
        ...
        <div className=" hidden relative inset-x-0 mt-[-35px] bg-white  rounded-full max-w-[1280px] h-[80px] mx-auto p-5 z-20 lg:flex justify-center ">
          ...
          <nav className="w-[70%] mr-[20px] flex justify-end gap-20 my-auto">
            <div className="flex">
              {USERPRO_NAVBAR_ELEMENTS.map((link, index) => (
                <Fragment key={index}>
                  ...
                  {/*{index !== USERPRO_NAVBAR_ELEMENTS.length - 2 && (
      ...
      {mobileNav === true && (
        <div className="lg:hidden bg-[#fab516] min-h-screen p-4 ">
          <ul className="h-full gap-12 lg:flex flex-col p-4">
            {USERPRO_MOBILE_NAVBAR_ELEMENTS.map((link, index) => (
