...
export function UserAuthForm({ className, ...props }: UserAuthFormProps) {
  ...
  const [errorPercentage375, setErrorPercentage375] = React.useState<
    string | undefined
  >("");
  const [errorPercentage500, setErrorPercentage500] = React.useState<
    string | undefined
  >("");
  ...
  const onSubmit = async (values: z.infer<typeof RegisterSchema>) => {
    ...
    // Check remise values before submission
    // Check remise values before submission
    if ((values?.sales375 ?? 0) > 0.95) {
      console.log("entered error log for sales375");
      setErrorPercentage375("Il faut saisir une valeur entre 0 et 0.95");
      setPending(false);
      return; // Prevent submission if the validation fails
    }

    if ((values?.sales500 ?? 0) > 0.95) {
      console.log("entered error log for sales500");
      setErrorPercentage500("Il faut saisir une valeur entre 0 et 0.95");
      setPending(false);
      return; // Prevent submission if the validation fails
    }
      ...
      fetch(`${process.env.NEXT_PUBLIC_API_URL}/auth/signup`, {
        ...
        .then((data) => {
          if (data.success) {
            ...
            setPending(false);
            window.location.href = "/admin/liste-des-clients";
          } else {
            setError(data.error);
            setPending(false);
          }
            ...
            <div className="grid lg:grid-cols-2 grid-cols-1 gap-8">
                      ...
                      <FormControl>
                        <Input
                          ...
                          onChange={(e) => {
                            ...
                            if (value.length <= 10) {
                              field.onChange(value); // Updates the field only if the input is valid
                            }
                          }}
                        />
                      </FormControl>
              ...
              <div className="grid gap-1">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Prix de vente Kit avec panneaux 375</FormLabel>
                      <FormControl>
                        <Input
                          {...field}
                          disabled={isPending}
                          placeholder=""
                          type="text" // Keep type as text to handle input validation
                          onChange={(e) => {
                            const value = e.target.value;
                            // Allow numeric values with one decimal point (e.g., 0.95, 10.99, 0.60)
                            if (/^\d*\.?\d*$/.test(value)) {
                              field.onChange(e); // Update form value only if it's valid
                            }
                          }}
                        />
                      </FormControl>
                ...
                />
                {errorPercentage375 && (
                  <p className="text-red-500">{errorPercentage375}</p>
                )}
              </div>
              <div className="grid gap-1">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Prix de vente Kit avec panneaux 500</FormLabel>
                      <FormControl>
                        <Input
                          {...field}
                          disabled={isPending}
                ...
                />
                {errorPercentage500 && (
                  <p className="text-red-500">{errorPercentage500}</p>
                )}
              </div>
              <div className="grid gap-1">
                <FormField
            ...
            <div className="flex lg:justify-end items-end gap-4">
              ...
              <div className="col-start-2">
                <Button
                  ...
                  onClick={() => {
                    window.location.href = "/admin/liste-des-clients"; // Replace with your desired URL
                  }}
                >
                  <span
