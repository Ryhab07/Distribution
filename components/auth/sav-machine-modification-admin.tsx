...
export function SavMachinesFormModificationAdmin({
  className,
  id,
  ...props
  ...
  console.log("userData.dateDemande", userData);
  ...
  const onSubmit = async (values: z.infer<typeof SavMachinesSchema>) => {
    try {
      ...
      if (step === 1) {
        ...
        // Iterate over combinedData and append files or other fields to FormData
          ...
          if (files.length === 0) {
            ...
            files.forEach((file) => {
              if (typeof file === "string") {
                ...
                console.log(
                  `Appended existing file URL: ${fieldName} = ${file}`
                );
              } else {
        ...
        appendFilesToFormData("picture1", values.picture1 || []);
        ...
        startTransition(() => {
          ...
          const requestOptions: RequestInit = {
            method: method,
            body: formDataToSend, // Always send FormData for consistency
          };
  ...
  const handleRemovePicture1 = (index: number) => {
    const existingFiles = form.getValues("picture1");
    console.log("picture1 files before removal:", existingFiles);

    if (!Array.isArray(existingFiles)) {
      console.error(
        `Field "picture1" does not contain an array.`,
        existingFiles
      );
      return;
    }
    ...
    if (updatedFiles.length === 0) {
      form.setValue("picture1", []);
    } else {
      form.setValue("picture1", updatedFiles);
    }
  };
  ...
  const handleFileChange = (e, field) => {
    const newFiles = Array.from(e.target.files); // Get the new files
    const existingFiles = form.getValues(field) || []; // Get the existing files
  ...
  const removeFile = (field: "picture1" | "picture2", index: number) => {
    const files = [...form.getValues(field)];
    console.log("removing the files", files);
  ...
  const dateDeLaDemande2 = pending
    ? "" // Show an empty string while loading
    : userData[0]?.dateDemande &&
      !isNaN(new Date(userData[0]?.dateDemande).getTime())
    ? formatDate2(userData[0]?.dateDemande)
    : formatDate2(userData[0]?.created_at);

  console.log("userData[0]?.created_at", userData[0]?.created_at);
  console.log("userData[0]?.dateDemande", userData[0]?.dateDemande);
  console.log("dateDeLaDemande2", dateDeLaDemande2);
      ...
      >
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <h4> Date de la demande: {pending ? " " : dateDeLaDemande2}</h4>
            <div className="grid lg:grid-cols-2 grid-cols-1 gap-8">
              ...
              <div className="grid gap-1">
                ...
                <datalist id="installateur">
                  {entreprises.map((article, id) => (
                    <option key={id} value={article}>
                      {article}
                    </option>
                  ))}
                </datalist>
              ...
              <div className="grid gap-1">
                ...
                <datalist id="articleNumbers">
                  {articles.map((article) => (
                    <option key={article._id} value={article.NArticle}>
                      {article.Description}
                    </option>
                  ))}
                </datalist>
              ...
              <div className="grid gap-1">
                ...
                <datalist id="descriptions">
                  {articles.map((article) => (
                    <option key={article._id} value={article.Description}>
                      {article.NArticle}
                    </option>
                  ))}
                </datalist>
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <Popover
                          ...
                          onOpenChange={(isOpen) =>
                            toggleOpen("dateDemandeAvoirFournisseur", isOpen)
                          }
                        >
                          <PopoverTrigger asChild className="w-full">
                            <Button
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <Popover
                          ...
                          onOpenChange={(isOpen) =>
                            toggleOpen("dateReceptionAvoirFournisseur", isOpen)
                          }
                        >
                          <PopoverTrigger asChild className="w-full">
                            <FormControl>
              ...
              <div className="grid gap-1">
                <FormItem>
                  ...
                  <div className="bg-[#ffffff] p-10 rounded-md">
                    ...
                    <div className="mt-4">
                      {(!form.getValues("picture1") ||
                        form.getValues("picture1").length === 0) && (
                    ...
                    {/* New Button to Add More Documents */}
